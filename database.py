"""
PhysioAssist Database Manager v2
...
def init_db():
    ...
    # Migration: add has_rated column if missing
    try:
...
def update_user_email(telegram_id: int, email: str):
    conn = get_conn()
    conn.execute("UPDATE users SET email=? WHERE telegram_id=?", (email, telegram_id))
    conn.commit()
    conn.close()
...
def save_assessment(telegram_id: int, lang: str, body_region: str,
                    diagnosis: str, pain_scale: int, paid: int, report_text: str):
    conn = get_conn()
    conn.execute("""
...
def use_one_time_code(code: str, user_id: int) -> bool:
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM referral_codes WHERE code=? AND is_used=0", (code,)
    ).fetchone()
    if row:
        conn.execute("""
        ...
        """, (user_id, code))
        conn.execute(
            "UPDATE users SET points=points+25 WHERE telegram_id=?",
            (row["created_by"],)
        )
        conn.commit()
        conn.close()
...
def record_referral_share(sharer_id: int, new_user_id: int = None):
    """Award 5 points immediately when user shares."""
    add_points(sharer_id, 25)
    conn = get_conn()
    conn.execute("""
        INSERT INTO share_events (telegram_id, points_earned, new_user_id)
        VALUES (?, 5, ?)
...
def process_start_referral(new_user_id: int, ref_code: str):
    """When new user starts via referral link — give referrer +10 points."""
    conn = get_conn()
...
def mark_user_rated(telegram_id: int):
    conn = get_conn()
    conn.execute(
        "UPDATE users SET has_rated=1 WHERE telegram_id=?", (telegram_id,)
    )
    conn.commit()
    conn.close()


def has_user_rated(telegram_id: int) -> bool:
    conn = get_conn()
    user = conn.execute(
        "SELECT has_rated FROM users WHERE telegram_id=?", (telegram_id,)
    ).fetchone()
    conn.close()
    return bool(user and user["has_rated"])
...
def get_stats() -> dict:
    ...
    stats = {
        ...
        "total_ratings":     conn.execute("SELECT COUNT(*) FROM users WHERE has_rated=1").fetchone()[0],
    }
    conn.close()
    return stats
