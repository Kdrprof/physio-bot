...
- +5 pts immediately on share, +10 when friend joins
"""
...
PRICE_STARS      = 150
PRICE_LABEL      = "$1.99"
POINTS_PER_SHARE = 5     # immediate on press Share
POINTS_PER_JOIN  = 10    # when friend actually joins via link
POINTS_FOR_FREE  = 50
...
async def _do_analysis(message, context, paid=0):
    ...
    # Telegram Reviews prompt — first assessment only, not rated yet
    if tg_id and not has_user_rated(tg_id):
...
async def _show_share_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Core share logic — awards +5 pts immediately."""
    query = update.callback_query
    message = query.message if query else update.message
    lang  = context.user_data.get("lang", "en")
    tg_id = context.user_data.get("tg_id")
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            FINISHED: [
                CallbackQueryHandler(handle_btn,
                    pattern="^(pdf_full|pdf_quick|share|new_assessment|my_points)$"),
            ],
        },
